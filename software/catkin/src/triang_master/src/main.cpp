...
//static bool UDP_WAITFORDATA_FLAG = false;
...
static void ros_interface_thread(std::atomic<bool>& program_is_running, std::vector<std::string> &rx_master, std::vector<std::string> &rx_slave, std::atomic<bool> & udp_waitForData_Flag,  std::atomic<bool> & allow_tx);
void sub_thread();
...
void  send_master_id(std::atomic<int>& id_tx, std::atomic<bool>& allow_tx,  std::atomic<bool> & udp_waitForData_Flag);
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	atomic<int> id_tx { 0 };
	atomic<bool> udp_waitForData_Flag {false};
  ...
  //v_a.push_back(a_i);
	...
	std::vector<std::string> rx_master { "/" };
	std::vector<std::string> rx_slaves { "/" };
	//rx_master = &rx_master_buffer[0];
	//rx_slaves = &rx_slaves_buffer[0];

	udp_conv_rx udp;
	(void)udp.udp_init(41823,addr1,fd1);
	std::thread update_thread( udp.loop, std::ref(rx_master), std::ref(running), std::ref(addr1),std::ref(fd1));
	udp_conv_rx udp2;
	(void)udp2.udp_init(40000,addr2,fd2);
	std::thread update_thread_rx2( udp2.loop, std::ref(rx_slaves), std::ref(running), std::ref(addr2),std::ref(fd2));
  ...
  while(input_key!=27){
    ...
    //input_key = getch();
		//v_a.push_back(a_i);
		//size_of_vec = v_a.size();
		//std::cout << "\nRX SIZE: " << size_of_vec;
		//std::cout << "\nRX SIZE: " << rx_master.size();
...
static void ros_interface_thread(std::atomic<bool>& program_is_running, std::vector<std::string> &rx_master, std::vector<std::string> &rx_slave, std::atomic<bool> & udp_waitForData_Flag, std::atomic<bool>& allow_tx){
	...
	rviz_loop(program_is_running, rx_master, rx_slave, udp_waitForData_Flag, allow_tx);
}
